...
class FLIError(Exception):
    def __init__(self, msg, errno=None):
        super().__init__(msg)
        # negative system errno as returned by libfli, e.g. -32 == EPIPE
        # (USB pipe broken, device disconnected mid-transfer)
        self.errno = errno
...
class FLILibrary:
    ...
    def getDll(
        ...
        if FLILibrary.__dll is None:
            # an explicit path through the environment overrides the search
            env_path = os.environ.get("FLI_SDK_PATH") or os.environ.get("FLI_LIB_PATH")
            if env_path:
                FLILibrary.__dll = cdll.LoadLibrary(env_path)
                ...
                try:  # first try to load library from package directory
                    ...
                    try:
                        ...
                        # last resort: ask the platform linker (finds e.g.
                        # Debian's libfli.so.2 when the -dev symlink is absent)
                        found = ctypes.util.find_library("fli")
                        if not found:
                            raise
                        FLILibrary.__dll = cdll.LoadLibrary(found)
        ...
        if debug:
            ...
            host = c_char_p(DEBUG_HOST_FILENAME.encode())
